...
def pinn_train(PINN, nu, xt_resid, IC_xt, IC_u, BC_xt, BC_u, 
               f_hat, epochs_pinn, lr_pinn, tol):
    
    losses = [PINN.loss(xt_resid, IC_xt, IC_u, BC_xt, BC_u, f_hat).item()] 
    ep = [0]
    optimizer = torch.optim.Adam(PINN.parameters(), lr=lr_pinn)

    print(f"Epoch: 0 | Loss: {losses[0]}")
    for i in range(1, epochs_pinn+1):
        loss_values = PINN.loss(xt_resid, IC_xt, IC_u, BC_xt, BC_u, f_hat) 
