from torch import cos, linspace, meshgrid, hstack, zeros, vstack

def initial_u(x, t=0):
    return x

def initial_dudt(x, t=0):
    return 0*x
...
def ICBC_data(Xi, Xf, Ti, Tf, BC_pts, IC_pts):
    ...
    ##########################################################
    IC_x  = X_IC[:,0].unsqueeze(1)
    IC_t  = zeros(IC_x.shape[0], 1)
    IC_u1 = initial_u(IC_x)     
    IC_u2 = initial_dudt(IC_x) 
    IC    = hstack((IC_x, IC_t))
    ##########################################################
    BC_bottom_x = X_BC[0,:].unsqueeze(1) 
    BC_bottom_t = T_BC[0,:].unsqueeze(1)
    BC_bottom_u = boundary_bottom(BC_bottom_t)
    BC_bottom   = hstack((BC_bottom_x, BC_bottom_t)) 
    ##########################################################
    BC_top_x = X_BC[-1,:].unsqueeze(1)
    BC_top_t = T_BC[-1,:].unsqueeze(1)
    ...
    ##########################################################
    return IC, IC_u1, IC_u2, xt_train_BC, u_train_BC  
